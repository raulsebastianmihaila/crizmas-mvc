import {isObj, isFunc, isThenable, resolveThenable} from './utils.js';
...
const getPropRegularObject = (obj, prop) => {
  ...
  if (descriptor && isObj(descriptor.value) && !isThenable(descriptor.value)) {
...
export const observe = (observable, {key, preventApply} = {}) => {
...
export const apply = (target, thisArg, args, {key} = {}) => {
  programNotification();
...
export const ignore = (observable) => {
...
export const construct = (target, args, newTarget, {isRoot} = {}) => {
  programNotification();
...
export const unroot = (observable) => {
  if (isFunc(observable)) {
...
export const addObservedChild = (obj, child) => {
...
export const isObservedObject = (value) => {
...
export const removeObservedChild = (obj, child) => {
  if (!isObservedObject(obj)) {
